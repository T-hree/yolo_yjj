...
class SpaceToDepth(nn.Module):
    """
    这是核心手术刀：把空间维度(H, W)的信息切分到通道维度(C)去。
    """

    def __init__(self, dimension=1):
        super().__init__()
        self.d = dimension
        self.block_size = 2  # 默认切成2x2的小块

    def forward(self, x):
        # x 的形状是 [Batch, Channel, Height, Width]
        # 我们要把它切成4份，分别取出来
        return torch.cat([x[..., ::2, ::2],  # 0,0 位置
                          x[..., 1::2, ::2],  # 1,0 位置
                          x[..., ::2, 1::2],  # 0,1 位置
                          x[..., 1::2, 1::2]  # 1,1 位置
                          ], 1)  # 在通道维度拼接
...
class SPDConv(nn.Module):
    """
    这是我们将要调用的模块：先做 SpaceToDepth，再接一个卷积调整通道数。
    """

    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        super().__init__()
        # 因为 SpaceToDepth 把 H,W 减半了，通道数变成了原来的4倍
        # 所以这里的输入通道数实际上是 c1 * 4
        self.gs = SpaceToDepth()
        self.conv = Conv(c1 * 4, c2, k, s, p, g, d, act)

    def forward(self, x):
        return self.conv(self.gs(x))
